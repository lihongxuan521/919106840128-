...
playbyperson::playbyperson(QWidget *parent) : QWidget(parent)
{
    setMouseTracking(true);
    this->setWindowTitle("人人模式");
    setFixedSize(//设置窗口大小
    ...
    connect(&back,&QPushButton::clicked,this,&playbyperson::sendslot);      //点击这个按钮返回到主页面
    connect(&startnew,&QPushButton::clicked,this,&playbyperson::anewplay);  //点击这个按钮执行anewplay函数
}
void playbyperson::paintEvent(QPaintEvent *event)
{
    QPainter painter(this);
   //painter.setRenderHint(QPainter::Antialiasing,true);//抗锯齿
    QPixmap pix;
    pix.load(":/new/prefix1/w1tu.jpg");//加载图片
...
void playbyperson::mouseMoveEvent(QMouseEvent *event)
{
    int x=event->x();
...
void playbyperson::mouseReleaseEvent(QMouseEvent *event)
{
    if(selectpos==false){
        return;            //如果此处不能下棋 不落子
    }  else{
...
void playbyperson::sendslot(){
    emit mysignal();
}

void playbyperson::anewplay(){
