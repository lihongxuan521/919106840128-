...
#include"QMessageBox"
#include"QTimer"
playwithaijin::playwithaijin(QWidget *parent) : QWidget(parent)
{
    setMouseTracking(true);
    this->setWindowTitle("人机禁手模式");
    setFixedSize(//设置窗口大小
    ...
    connect(&back,&QPushButton::clicked,this,&playwithaijin::sendslot);
    connect(&startnew,&QPushButton::clicked,this,&playwithaijin::anewplay);
}

void playwithaijin::paintEvent(QPaintEvent *event)
{
...
void playwithaijin::initGame()
{
    game=new GameModel;
    initAIGame();
}
void playwithaijin::initAIGame()
{
    game_type=AI;
    game->gamestatus=playing;
    game->startgame(game_type);
}
void playwithaijin::mouseMoveEvent(QMouseEvent *event)
{
    int x=event->x();
...
void playwithaijin::mouseReleaseEvent(QMouseEvent *event)
{
    if(selectpos==false){
        return;            //如果此处不能下棋 不落子
    }  else{
...
void playwithaijin::anewplay(){
    game->gamestatus=dead;
    initGame();

}

void playwithaijin::sendslot(){
    emit mysignal();
}
