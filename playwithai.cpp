...
#include"QMessageBox"
#include"QTimer"
playwithai::playwithai(QWidget *parent) : QWidget(parent)
{
    setMouseTracking(true);
    this->setWindowTitle("人机模式");
    setFixedSize(//设置窗口大小
    ...
    connect(&back,&QPushButton::clicked,this,&playwithai::sendslot);
    connect(&startnew,&QPushButton::clicked,this,&playwithai::anewplay);

}

void playwithai::paintEvent(QPaintEvent *event)
{
...
void playwithai::initGame()
{
    game=new GameModel;
    initAIGame();
}
void playwithai::initAIGame()
{
    game_type=AI;
    game->gamestatus=playing;
    game->startgame(game_type);
...
void playwithai::mouseReleaseEvent(QMouseEvent *event)
{
    if(selectpos==false){
        return;            //如果此处不能下棋 不落子
    }  else{
...
void playwithai::anewplay(){
    game->gamestatus=dead;
    initGame();

}

void playwithai::sendslot(){
    emit mysignal();
}
